...
public class EstadoBatallaNPC extends Estado {
	...
	public EstadoBatallaNPC(Juego juego, PaqueteBatalla paqueteBatalla) {
		...
		miniaturaEnemigo = Recursos.personaje.get("Minotauro").get(5)[0];
			...
			if (juego.getHandlerMouse().getNuevoClick()) {
				...
				if (haySpellSeleccionada && seRealizoAccion) {
					if (!enemigo.estaVivo()) {
						...
						paqueteAtacar = new PaqueteAtacar(paquetePersonaje.getId(),paqueteEnemigo.getId(), personaje.getSalud(),personaje.getEnergia(), enemigo.getSalud(),
	...
	public void graficar(Graphics g) {
		...
		g.drawImage(Recursos.personaje.get("Minotauro").get(7)[0], 550, 75, 256, 256, null);
	...
	private void finalizarBatalla() {
		try {
			...
			/**
			 * Comparo solo si subio el jugador de nivel para asignarle los 3 puntos correspondientes
			 */
